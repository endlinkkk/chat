...
async def test_create_chat_command_succes(
    chat_repository: BaseChatRepository, mediator: Mediator, faker: Faker, user: User
):
...
async def test_create_message_command_success(
    ...
    message_from_repo: Message = await message_repository.get_message_by_message_oid(
        message.oid
    )
...
async def test_get_chat_messages(
    ...
    messages, *_ = await mediator.handle_command(
        GetUserChatMessagesCommand(user=user, chat_oid=chat.oid)
    )
